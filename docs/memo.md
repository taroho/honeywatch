...
## 管理 SSH を 2200 に移動し 22番を Honeypot へ

SSH 攻撃を観測するため、22番を SSH Honeypot に明け渡す。
管理用 SSH（OS の sshd）は 2200番に移動する（SSM は保険として残す）。

当初 5555 を検討したが、利用中ネットワークで 5555 の outbound が塞がれていたため
2200 に変更した。

### 変更点

- docker-compose.yml: Honeypot の SSH を `0.0.0.0:22:2222`（ホスト22 → コンテナ2222）に変更
- EC2 の ssh.socket: `ListenStream=0.0.0.0:2200` に変更（管理 SSH を移動）
  ※ Ubuntu 22.04+ は socket activation のため sshd_config の Port だけでは効かない
- Security Group: 22番を `0.0.0.0/0` に全開放、2200番を自分の IP のみで許可

### 移行手順（締め出し防止のため順番厳守）

1. ssh.socket に 22 と 2200 を両方書いて再起動（両方で入れる状態に）
2. Security Group に 2200番（自IP）を追加
3. `ssh -p 2200` で入れることを確認（ここで入れないまま進むと締め出される）
4. ssh.socket から 22 を削除して 2200 のみにし、再起動
5. コード反映（docker compose up -d honeypot）
6. Security Group で 22番を 0.0.0.0/0 に変更

### ハマった点

- EC2 停止→起動でパブリック IP が変わり、古い IP を叩いて全ポート timed out になった。
  → 接続前に必ず現在の IP を確認する。恒久 IP が必要なら Elastic IP を割り当てる。
- Ubuntu の ssh.socket が Port を管理しており、sshd_config だけでは 2200 が効かなかった。
...
- Dashboard 閲覧の SSH トンネルも 2200 経由になる:
  `ssh -p 2200 -i <キー>.pem -L 3000:localhost:3000 -L 8000:localhost:8000 ubuntu@<IP>`
