...
- サーバー側の認証情報は環境変数で設定（ハードコードしない）

### フロントエンドの認証情報

- Dashboard はログインフォームで認証情報を入力させる
- 認証情報はソースコードに埋め込まない（ハードコード禁止）
- 入力値は base64 エンコードして localStorage に保持し、API 呼び出し時に付与する
- API が 401 を返した場合は認証情報を破棄してログイン画面に戻す
